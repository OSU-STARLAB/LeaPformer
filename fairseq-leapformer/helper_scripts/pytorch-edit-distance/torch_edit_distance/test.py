...
class EditDistanceTest(unittest.TestCase):
    ...
    def test_wer(self):
        ...
        x = torch.tensor([1, 0, 1, 1, 2, 1, 3], dtype=torch.int).cuda().reshape(1,-1)
        y = torch.tensor([0, 1, 2, 1, 2, 1, 3], dtype=torch.int).cuda().reshape(1,-1)

        z = torch.tensor([0, 1, 0, 3], dtype=torch.int).reshape(1,-1)
        #z = torch.tensor([[0, 1, 0, 3], [0, 0, 1, 1]], dtype=torch.int)

        #n = torch.tensor([7, 5], dtype=torch.int).cuda()
        #m = torch.tensor([7, 3], dtype=torch.int).cuda()
        n = torch.tensor([7], dtype=torch.int).cuda()
        m = torch.tensor([7], dtype=torch.int).cuda()

        print(f"strings: {x}, {x.shape}, {y}, {y.shape}", flush=True)
        print(f"lens: {n}, {n.shape}, {m}, {m.shape}", flush=True)
        print(f"other: {self.blank}, {self.separator}", flush=True)
